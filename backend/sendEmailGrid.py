...
def moodle_user_create(email,firstname,lastname):
    ...
    try:
        ...
        if 'exception' in json.loads(response.text):
            ...
            message = Mail(
            from_email='praful1611@outlook.com', 
            to_emails= email,
            subject='Welcome to the Bootcamp' + firstname + 'username:'+ username + 'Password: P@sssword', 
            html_content='<strong>Please login using your credentials</strong>' )                        
